...
MEDIA_COLUMNS = [
    ...
    "canvas_id", "canvas_name", "thumb_w", "thumb_h", "has_subtitles",
]
    ...
    try:
        ...
        for item in items:
            ...
            media_rows.append((
                ...
                item.get("canvas_id"), item.get("canvas_name"), tw, th,
...
def facets(db_path: str | Path, stashed: Iterable[str] = ()) -> dict[str, Any]:
    ...
    try:
        ...
        return {"tags": tags, "models": models, "canvases": canvases, "total": total}
    finally:
        conn.close()
