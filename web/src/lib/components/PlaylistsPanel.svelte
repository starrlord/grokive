    ...
    <div class="flex flex-col gap-1">
      {#each $playlists as pl (pl.id)}
        ...
        <div class="flex items-center gap-1.5 rounded-lg border border-line p-1.5">
          <button class="grid h-7 w-7 shrink-0 place-items-center rounded bg-[var(--accent)] text-white" title="Play" onclick={() => onplay(pl)}>▶</button>
          ...
          <button class="grid h-7 w-7 shrink-0 place-items-center rounded border border-line text-muted transition hover:border-red-500/50 hover:bg-red-500/10 hover:text-red-400" title="Delete" onclick={() => (confirming = pl)} aria-label="Delete playlist">
