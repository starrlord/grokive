      ...
      <div class="mb-5">
        ...
        <div class="flex flex-wrap items-baseline gap-2">
          {#each shown as t (t.name)}
            <button class="rounded-full px-2.5 py-1 leading-none transition {$filters.tags.includes(t.name) ? 'bg-[var(--accent)] text-white' : 'border border-line hover:border-[var(--accent)]'}"
        ...
        <div>
          ...
          <div class="flex flex-wrap gap-2">
            {#each facets.models as m (m.name)}
              <button class="rounded-full px-3 py-1 text-sm {$filters.models.includes(m.name) ? 'bg-[var(--accent)] text-white' : 'border border-line'}" onclick={() => toggleModel(m.name)}>{m.name} <span class="opacity-60">{m.count}</span></button>
            {/each}
          </div>
        </div>
      {/if}
