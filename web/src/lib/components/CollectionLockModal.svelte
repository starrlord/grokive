<script>
  ...
  let m = $state(mode);          // current sub-mode (unlock <-> force can switch)
  let pw = $state('');
  let confirm = $state('');
  ...
  const doSet = () => {
    ...
    run(() => lockCollection(collection.id, pw));
  };
  const doUnlock = () => run(() => unlockCollection(collection.id, pw));
...
<div use:portal class="fixed inset-0 z-[80] grid place-items-center bg-[var(--overlay-strong)] p-4 backdrop-blur-sm" role="presentation"
  ...
  <div class="relative w-full max-w-sm overflow-hidden rounded-2xl border border-line bg-[var(--surface-solid)] shadow-[0_30px_80px_-20px_rgba(0,0,0,0.7)]"
    ...
    <div class="p-5">
      {#if m === 'manage'}
        {#if confirmRemove}
          <p class="mb-4 text-sm text-muted">Remove the password from this collection? Its media will reappear everywhere.</p>
        ...
        {:else}
          <p class="mb-4 text-sm text-muted">This collection is unlocked. You can re-lock it now or remove its password entirely.</p>
